...
export class ColorBarView extends BaseColorBarView {
  ...
  protected _scanning_binning(color_mapper: ScanningColorMapper): Arrayable<number> {
    let {binning, force_display_low} = color_mapper.metrics
    const {display_high} = this.model
    let {display_low} = this.model

    if (force_display_low && (display_low == null || color_mapper.metrics.min > display_low)) {
      // ScanningColorMapper overrides the display_low, for example when EqHistColorMapper
